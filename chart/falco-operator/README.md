...
The following table lists the configurable parameters of the falco-operator chart v0.4.0-rc2 and their default values. See [values.yaml](values.yaml) for the full list.
...
| mtls.caCertDuration | string | `"8760h"` | Duration for the bootstrap CA certificate (only used when createIssuer is true). cert-manager renews it automatically; private-key replacement depends on its rotation policy and cert-manager version. CA key rotation requires coordinated trust-bundle and leaf-certificate updates, which this chart does not automate. |
