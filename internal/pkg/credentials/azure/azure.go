...
// RegistryConfig.TLS.InsecureSkipVerify when set. Mirrors puller.go's own existing inline
// transport-building pattern -- no shared helper, this is the only caller here.
func exchangeHTTPClient(registryOpts *puller.RegistryOptions) *http.Client {
	if registryOpts != nil && registryOpts.InsecureSkipVerify {
		tlsConfig := &tls.Config{InsecureSkipVerify: registryOpts.InsecureSkipVerify} //nolint:gosec // user-configured
		httpTransport := &http.Transport{TLSClientConfig: tlsConfig}
		return &http.Client{Transport: retry.NewTransport(httpTransport)}
	}
