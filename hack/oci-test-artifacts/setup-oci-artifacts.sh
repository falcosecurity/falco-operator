...
# Run once before the chainsaw suite: make registry.setup
#
# CI runs its own copy of this script (.github/e2e/oci-artifacts/push-artifacts.sh) instead of
# this one, so it has no dependency on hack/ or the Makefile. Keep both copies in sync by hand
# when the test artifact set changes.
#
# Prerequisites: falcoctl, kubectl, tar in PATH.
# The registry must already be deployed: kubectl apply -f hack/oci-test-artifacts/registry.yaml
set -euo pipefail
...
# own tempdir instead, so this works for any non-root user too.
